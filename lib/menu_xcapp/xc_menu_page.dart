...
/// Model: saved notification (simple)
...
class _XcMenuPageState extends State<XcMenuPage> with SingleTickerProviderStateMixin {
  ...
  Future<void> _loadPrefs() async {
    try {
      ...
      // selected apps - try getStringList first (Flutter), fallback to JSON raw (native-style)
      ...
      if (selList != null && selList.isNotEmpty) {
        ...
        // fallback: native might have saved JSON string under same key
      ...
      if (_nativeEnabledFlag && !_listening) {
        _listening = true; // UI indicator (native handles events when flutter closed)
      }
  ...
  /// IMPORTANT:
  /// Save selected apps in *both* Flutter-friendly StringList *and* JSON raw string.
  /// - Flutter uses setStringList
  /// - Native Kotlin fallback reads raw JSON via prefs.getString(...)
  Future<void> _saveSelectedApps() async {
    final prefs = await SharedPreferences.getInstance();
    final list = _selectedPackageNames.toList();
    // 1) Flutter-friendly
    await prefs.setStringList(kPrefsSelectedApps, list);
    // 2) Native-friendly JSON fallback
    try {
  ...
  // start native foreground (MethodChannel) + flutter stream while app is alive
  ...
  Future<void> _startListening() async {
    ...
    // show persistent local notif too (optional)
    await NotificationHelper.showPersistent(_persistentNotificationId, 'XC Listener aktif', 'Menangkap notifikasi');

    // start Flutter-level stream for live UI (works while Flutter app alive)
    _sub?.cancel();
    _sub = NotificationListenerService.notificationsStream.listen(
      (ServiceNotificationEvent event) async {
        try {
          ...
          // IMPORTANT: only process if package is selected (if selection not empty)
          ...
          // show a one-shot local notification for the event
