...
// Diperbarui: perbaikan start untuk "start & close app" agar native foreground service dimulai dulu
// lalu UI ditutup hanya jika native menyatakan berhasil. Tambahan: lifecycle observer,
// pengecekan permission, dan handling MethodChannel result dengan boolean.
import 'dart:async';
import 'dart:convert';
...
class _XcMenuPageState extends State<XcMenuPage> with SingleTickerProviderStateMixin, WidgetsBindingObserver {
...
// reload prefs when app resumes (native may have changed enabled flag)
...
```
  ...
  // only reflect UI state; actual native service state controlled by native side
  if (_nativeEnabledFlag) {
    _listening = true;
  }
...
Future<bool> _startNativeForeground() async {
if (!Platform.isAndroid) return false;
try {
final bool? res = await _serviceChannel.invokeMethod<bool>('startForeground');
log('startForeground result: $res');
return res ?? true;
} on PlatformException catch (e) {
log('startForeground PlatformException: ${e.message}');
return false;
} catch (e) {
...
Future<bool> _stopNativeForeground() async {
if (!Platform.isAndroid) return false;
try {
final bool? res = await _serviceChannel.invokeMethod<bool>('stopForeground');
log('stopForeground result: $res');
return res ?? true;
} on PlatformException catch (e) {
log('stopForeground PlatformException: ${e.message}');
return false;
} catch (e) {
...
```
// subscribe to flutter stream only while app alive
_sub?.cancel();
_sub = NotificationListenerService.notificationsStream.listen(
  (ServiceNotificationEvent event) async {
    try {
      ...
      // update persistent notif text/count (flutter-side backup)
      await NotificationHelper.showPersistent(_persistentNotificationId, 'XC Listener aktif (${_savedNotifications.length})',
...
/// Start native foreground (and save prefs) then close the Flutter UI
/// Only close app if native returns success = true.
Future<void> _startAndCloseApp() async {
// 1) ensure notification access
bool granted = false;
try {
...
// requestPermission opens settings; user must manually grant; plugin returns true if navigated and granted.
if (!opened) {
if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Notification access not granted')));
return;
}
// after user returns from settings, re-check
granted = await NotificationListenerService.isPermissionGranted();
if (!granted) {
if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please enable Notification Access in settings')));
return;
}
}
} catch (e) {
...
```
// 2) persist current settings so native can read them
await _saveSelectedApps();
await _savePostUrl();
await _saveEnabledFlag(true);

// 3) attempt to start native foreground service and wait for result
final bool nativeStarted = await _startNativeForeground();
if (!nativeStarted) {
  // if native failed, rollback enabled flag and inform user
  await _saveEnabledFlag(false);
  if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Failed to start native foreground service')));
  return;
}

// 4) start Flutter stream (while app alive) so UI sees events until we close
await _startListening();

// 5) show flutter persistent notif as backup (native should be authoritative)
await NotificationHelper.showPersistent(_persistentNotificationId, 'XC Listener aktif', 'Menangkap notifikasi');

// small delay to let native service settle
await Future.delayed(const Duration(milliseconds: 300));

// 6) close app UI only if native started successfully
try {
  SystemNavigator.pop(); // close UI; native service should continue
} catch (e) {
  log('SystemNavigator.pop error: $e');
  if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Could not close UI, but service started')));
}
```
...
const SizedBox(width: 8),
ElevatedButton.icon(onPressed: _listening ? null : _startAndCloseApp, icon: const Icon(Icons.play_arrow), label: const Text('Start & Close App')),
const SizedBox(width: 8),
...
separatorBuilder: (*, __) => const Divider(height: 0),
...
labelText: 'POST URL',
hintText: '[https://your-server.example/notify](https://your-server.example/notify)',
...
return '${dt.year}-${_two(dt.month)}-${_two(dt.day)} ${_two(dt.hour)}:${_two(dt.minute)}:${*two(dt.second)}';
} catch (*) {
return iso;
}
}
...
await _startAndCloseApp(); // start native then close app if native ok
}
},
